...
// ─── data.js — Fuente de datos global ─────────────────────────────────────
// status valores: 'crítico' | 'bajo stock' | 'óptimo' | 'sin stock'
...
// Normalizar status (quita tildes) para comparaciones internas
function _norm(s) {
  return (s || '').toLowerCase().replace(/[áéíóú]/g, function (c) {
...
function getAlerts() {
  return {
    critico:   ECO_PRODUCTS.filter(function (p) { return _norm(p.status) === 'critico'; }),
    bajoStock: ECO_PRODUCTS.filter(function (p) { return _norm(p.status) === 'bajo stock'; }),
    sinStock:  ECO_PRODUCTS.filter(function (p) { return _norm(p.status) === 'sin stock'; }),
  };
}
...
// Catálogo unificado para POS (usa los mismos datos que ECO_PRODUCTS)
const ECO_CATALOG = ECO_PRODUCTS.reduce(function (acc, p) {
  acc[p.id] = {
    name:     p.name,
    category: p.category,
    stock:    p.stock,
